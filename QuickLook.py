...
from scales_proctab import Proctab
...
def fits_writer_steps(
    ...
    *,
    ...
    print("+++++++++++ FITS file saved +++++++++++: %s", output_path)
    ...
    if proctab is not None:
        ...
        if frame is not None:
            ...
            use_frame = SimpleNamespace(header=header)
...
def run_quicklook(input_data,iswrite=True):
    print("+++++++++++ Quicklook Started +++++++++++")
    
    global proctab
    if "proctab" not in globals() or proctab is None:
        proctab = Proctab()
    ...
    #calib_path = Path.cwd() / "calib"
    calib_path = Path(__file__).resolve().parent / "calib"
    #calib_path = Path.cwd() / "quicklook" / "calib"
    #print(calib_path)    
    with fits.open(input_data) as hdul:
        ...
        if (
            ...
            if n_ext == 1:
                ...
                if data_1.ndim == 2:
                    print("Found a single frame.")
                    data_11 = swap_odd_even_columns(data_1,do_swap=False)
                    slope_filled1 = reference.reffix_hxrg(data_11, nchans=4, fixcol=True)
                    print("+++++++++++ ACN & 1/f Correction applied +++++++++++")
                    ...
                    if iswrite:
                        fits_writer_steps(
                            ...
                            proctab=proctab,
                            overwrite=True)
                    slope_filled1 = iterative_sigma_weighted_ramp_fit(
                        img_corr,
                        read_time=read_time)

                    t1 = time.time()
                    print(f"Ramp fitting finished in {t1-t0:.2f} seconds.")
                else:
                ...
                data_11 = swap_odd_even_columns(ramp3d,do_swap=True)
                img_corr = reference.reffix_hxrg(data_11, nchans=4, fixcol=True)
                print("+++++++++++ ACN & 1/f Correction applied +++++++++++")
                slope_filled1 = iterative_sigma_weighted_ramp_fit(
                    img_corr,
    ...
    cwd = os.getcwd()
    redux_path = os.path.join(cwd, "ql_redux")

    if obs_mode == "Im":
        print("BPM correction started")
        ...
        print("BPM correction completed")
        hdr['IMGFIT3'] = (redux_path, "Pipeline redux output directory")
        #hdr['IMGFITDISP'] = 'path'
        #hdr['IMGFITSEL'] = 3
        fits_writer_steps(
            data=slope_filled,
        ...
        slope_filled = np.array(slope_filled2).reshape(slope_filled1.shape)
        print("BPM correction completed")
        hdr['IFSFIT3'] = (redux_path, "Pipeline redux output directory")
        #hdr['IFSFITDISP'] = 'path'
        #hdr['IFSFITSEL'] = 3
        fits_writer_steps(
            data=slope_filled,
        ...
        if ifs_mode == "LowRes-K":
            ...
            if (
                ...
                cube1,error1 = optimal_extract_with_error(
                    R_matrix,
                    slope_filled,
                    read_noise_var)

                cube= cube1.reshape(56, 103, 110)
                hdr['IFSCUBE3'] = (redux_path, "Pipeline redux output directory")
                hdr['IFSCUBEDISP'] = 'path'
                hdr['IFSCUBESEL'] = 3
                fits_writer_steps(
                    data=cube,
                    header=hdr,
